...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="31%">&nbsp;</td>
	 ...
	 <tr align="left">
	   <td height="20" colspan="3"><span class="inserted">Select the campaign below to send emails</span> OR <a href="cron.php" class="mainmenu">Execute all Campaigns</a></td>
  </tr>
	 <tr>
	   <td colspan="3"><span class="info">Select the campaign you want to execute and click on send now . </span></td>
  </tr>
	 <tr>
	   <td colspan="3">&nbsp;</td>
  </tr>
	 <tr>
	   <td colspan="3"><table width="100%"  border="0" cellspacing="2" cellpadding="0" bgcolor="#EEEEEE">
		...
		$catid=$mysql->echo_one("select ".$table_prefix."email_advt_category.id from ".$table_prefix."email_advt_category,".$table_prefix."ea_cnc where ".$table_prefix."email_advt_category.id=".$table_prefix."ea_cnc.catid and ".$table_prefix."ea_cnc.campid=$row[0]");
		  ...
		  //$tot= mysql_query("select * from ".$table_prefix."email_advt a, ".$table_prefix."ea_em_n_cat b where a.id=b.eid AND a.unsubstatus=0 AND b.cid=$catid and b.eid<=$row[3]");
		//$sent= mysql_num_rows($tot);
		...
		$subAdminId=$mysql->echo_one("select aid from ".$table_prefix."campaign_access_control where cid=$row[0] and access_status=1");
				...
				//if(isset($_COOKIE['inout_sub_admin']))
			{
				...
				( SELECT eid FROM ".$table_prefix."admin_access_control where aid=$subAdminId )c  on b.cid=c.eid 
				 where a.unsubstatus=0  AND b.unsubstatus=0 and  a.id= b.eid and b.id<=$row[3]  ");
...
FROM ".$table_prefix."ea_em_n_cat a, ".$table_prefix."ea_extraparam b  inner join 
				( SELECT cid FROM ".$table_prefix."ea_cnc where catid=c.cid )c  on a.eid= b.eid
				 where    and b.id>$row[3] ")
				 */
			}
  ...
  ?>
             &nbsp;</td>
                 ...
                 <td align="center" valign="middle" style="border-bottom:1px solid #CCCCCC; "><a href="cron.php?id=<?php echo $row[0]; ?>" class="header">Send Now</a> </td>
               </tr>
               <?php $i+=1; 
}?>   
               <tr>
                 ...
                 <td>&nbsp;</td>
                 <td>&nbsp;</td>
                 <td align="center"><hr width="50%" size="1"></td>
                 <td align="center">&nbsp;</td>
               </tr>
               <tr>
                 <td>&nbsp;</td>
               ...
               <tr>
                 ...
                 <td>&nbsp;</td>
                 <td>&nbsp;</td>
                 <td colspan="3" align="center"><form name="form1" method="post" action="cron.php">
                   <input type="submit" name="Submit" value="Execute all Campaigns">
                 </form></td>
                </tr>
	 ...
	 <tr>
	   <td colspan="3"><span class="info"> </span></td>
  </tr>
	 <tr>
	   <td>&nbsp;</td>
	   <td>&nbsp;</td>
