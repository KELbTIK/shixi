...
<form method="post" name="applicationForm" action="" id="application-tracking-form">
	<input type="hidden" name="orderBy" value="{$orderBy|escape:'html'}" />
	<input type="hidden" name="appJobId" value="{$current_filter}" />
	<input type="hidden" name="order" value="{$order}" />
	<input id="action" type="hidden" name="action" value="" />
	<p class="action-form-buttons"><input type="submit" value="[[Approve selected]]" onclick="submitForm('approve'); return false;" /> &nbsp; <input type="submit" value="[[Reject selected]]" onclick="submitForm('reject')" /> &nbsp; <input type="submit" value="[[Delete]]" onclick="if (confirm('[[Are you sure you want to delete selected application(s)?]]')) submitForm('delete');" /></p>

	<div class="navigation-application">
		<div class="numberPerPage">
			[[Number of listings per page]]:
			<select class="phrases-per-page perPage" name="phrases_per_page" onchange="window.location = '?page=1&amp;appsPerPage='+ this.value +'&amp;score={$score}&amp;appJobId={$current_filter}&amp;orderBy={$orderBy|escape:'html'}&amp;order={$order}'">
	...
	<table border="0" cellpadding="0" cellspacing="0" class="tableSearchResultApplications" width="100%">
		...
		{foreach item=application from=$applications name=applications}
			<tr id = "trAppl_{$application.id}" {if !$application.note}style="border-bottom: 1px solid #B2B2B2;" {/if}>
				<td>&nbsp;</td>
				...
				<td class="ApplicationPointedInListingInfo">
					<span class="name">{$application.user.FirstName} {$application.user.LastName}</span>
					<br />
				...
				<td class="ApplicationPointedInListingInfo">
					...
					{if $application.comments}
						<a style="cursor: pointer; " onclick="showCoverLetter('{$application.id}')" href="#">[[View Cover Letter]]</a>
						<br />
				...
				<td class="ApplicationPointedInListingInfo">
					<span class="app-track-link"><a href="{$GLOBALS.site_url}/my-job-details/{$application.job.sid}/">{$application.job.Title}</a>
					</span>
				</td>
				{if $acl->isAllowed('use_screening_questionnaires')}
					{if $application.score > 0}
						<td class="ApplicationPointedInListingInfo"><a href="{$GLOBALS.site_url}/applications/view-questionnaire/{$application.id}">[[{$application.passing_score}]] ({$application.score})</a>
						</td>
					{else}
			...
			{if $application.note}
				<tr style="border-bottom: 1px solid #B2B2B2;">
					<td {if $acl->isAllowed('use_screening_questionnaires')}colspan="10"{else}colspan="9"{/if} class="ApplicationPointedInListingInfo">
			...
			{else}
				<tr id="trNote_{$application.id}" style="display: none">
					<td id="tdNote_{$application.id}" {if $acl->isAllowed('use_screening_questionnaires')}colspan="10"{else}colspan="9"{/if} >
						<div id="formNote_{$application.id}" class="form-notes">
						</div>
					</td>
				</tr>
