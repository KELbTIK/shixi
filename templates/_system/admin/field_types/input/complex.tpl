...
<div id='complexFields_{$complexField}' class="complex">
    {foreach from=$complexElements key="complexElementKey" item="complexElementItem"}
        ...
        {foreach from=$form_fields item=form_field}
            ...
            {elseif $form_field.id == "listings_range_from"}
                <fieldset style="max-width: 660px;">
                    <div class="inputName">{tr}{$form_field.caption}{/tr|escape:'html'}</div>
                    <div class="inputReq">&nbsp;{if $form_field.is_required}*{/if}</div>
                    <div class="inputField">[[from]] {input property=$form_field.id complexParent=$complexField complexStep=$complexElementKey} [[to]] {input property=listings_range_to complexParent=$complexField complexStep=$complexElementKey}</div>
                </fieldset>
