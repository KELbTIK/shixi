{if $GLOBALS.is_ajax || $smarty.get.ajax_submit}
	{foreach from=$errors key=key item=error}
		...
		{elseif $key == 'NOT_ACCEPTABLE_FILE_FORMAT'}
			<p class="error">[[Not acceptable file type]]</p>
	...
	{if !($complexField && $filesInfo.$complexStep.file_name ne null) && !(!$complexField && $value.file_name ne null)}
		<small>([[max.]] {$uploadMaxFilesize} M)</small>
	{/if}

	{if $complexField && $filesInfo[$complexStep].file_name ne null}
		<div id="file_{if $complexField}{$complexField}[{$id}][{$complexStep}]{else}{$id}{/if}" style="float:left;">
			<div class="complex-view-file-caption">
				{$filesInfo.$complexStep.file_name|escape:'html'} ({$filesize|string_format:"%.2f"} {$size_token})
