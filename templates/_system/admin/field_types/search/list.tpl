...
{else}
<select name='{$id}[equal]' {if $sort_by_alphabet}class="sortable-select"{/if}>
	{if $id != 'data_source'}
		<option value="">[[Any]] {tr}{$caption}{/tr|escape:'html'}</option>
	...
	{/if}
	{foreach from=$list_values item=list_value}
		<option value='{$list_value.id}' {if $list_value.id == $value.equal}selected="selected"{/if} >{tr}{$list_value.caption}{/tr|escape:'html'}</option>
	{/foreach}
</select>
{/if}