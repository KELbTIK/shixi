...
<script type="text/javascript">
    ...
    function changePermissions(user_group_sid)
    {
        if (user_group_sid) {
			...
			$.post(url, { 'user_group_sid': user_group_sid, 'product_type': '{$product_type}', 'permissions_type': 'additional', 'params': '{$params}'},  function(data){
				$("#additional_permissions").html(data);
			});
			if (access_settins == 1) {
				$.post(url, { 'user_group_sid': user_group_sid, 'product_type': '{$product_type}', 'permissions_type': 'access', 'params': '{$params}'},  function(data){
