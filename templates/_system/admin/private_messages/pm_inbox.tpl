		...
		<tbody>
			{foreach from=$message item=one}
				<tr class="{cycle values = 'evenrow,oddrow'}">
					<td><a href="{$GLOBALS.site_url}/private-messages/pm-read/?user_sid={$user_sid}&amp;mess={$one.id}&amp;from=in&amp;page={$page}">{$one.subject}</a></td>
					<td>{$one.from_name}</td>
