...
<div class="app-tracking sorting-filters">
	<form method="post" name="applicationFilter" action="" class="form-inline"  id="applicationFilter">
	    ...
	    <div class="form-group">
            ...
            <select class="form-control" name="appJobId">
		...
		{/if}
        <div class="form-group">
            <input type="submit" name="applicationFilterSubmit" value="[[Filter]]" class="button btn btn-dark" />
        </div>
	</form>
</div>
<form method="post" name="applicationForm" action="" class="form-inline">
    <input type="hidden" name="orderBy" value="{$orderBy|escape:'html'}" />
    <input type="hidden" name="order" value="{$order}" />
    <input type="hidden" name="appJobId" value="{$current_filter}" />
    <input id="action" type="hidden" name="action" value="" />
    <div class="navigation-application form-group">
        <div class="numberPerPage">
            <label>[[Number of listings per page]]: &nbsp;</label>
            <select class="phrases_per_page perPage form-control" name="phrases_per_page" onchange="window.location = '?page=1&amp;appsPerPage='+ this.value +'&amp;score={$score}&amp;appJobId={$current_filter}&amp;orderBy={$orderBy|escape:'html'}&amp;order={$order}'" >
    ...
    <div class="form-group pull-right btn-filter">
        <input class="btn btn-success" type="submit" value="[[Approve selected]]" onclick="submitForm('approve'); return false;" />
    </div>
    <div class="form-group pull-right btn-filter">
        <input class="btn btn-warning" type="submit" value="[[Reject selected]]" onclick="submitForm('reject')" />
    </div>
    <div class="form-group pull-right btn-filter">
        <input class="btn btn-danger" type="submit" value="[[Delete]]" onclick="if (confirm('[[Are you sure you want to delete selected application(s)?]]')) submitForm('delete');" />
    </div>
        ...
        <table class="tableSearchResultApplications table table-condensed">
            ...
            <tr>
                ...
                <th class="pointedInListingInfo2">
                    ...
                    {if $orderBy == 'applicant'}{if $order == 'asc'}<i class="fa fa-sort-up"></i>{else}<i class="fa fa-sort-down"></i>{/if}{/if}
                </th>
                ...
                {if $acl->isAllowed('use_screening_questionnaires')}
                    <th class="pointedInListingInfo2">
                        ...
                        {if $orderBy == 'score'}{if $order == 'asc'}<i class="fa fa-sort-up"></i>{else}<i class="fa fa-sort-down"></i>{/if}{/if}
                    </th>
                {/if}
                <th class="pointedInListingInfo2">
            ...
            {foreach item=application from=$applications name=applications}
                <tr id="trApplication_{$application.id}" {if !$application.note}class="table-application-border-bottom"{/if}>
                    ...
                    <td class="ApplicationPointedInListingInfo ApplicationActions" width="10%">
                        ...
                        <br />
                        {if $application.note != ''}
                            <a href="{$GLOBALS.site_url}/edit-notes/?apps_id={$application.id}" onclick="modifyNote( '{$application.id}', '{$GLOBALS.site_url}/edit-notes/?apps_id={$application.id}&amp;page=apps'); return false;" class="action">[[Edit Note]]</a>&nbsp;&nbsp;
                        {else}
                            <a href="{$GLOBALS.site_url}/add-notes/?apps_id={$application.id}" onclick="modifyNote( '{$application.id}', '{$GLOBALS.site_url}/add-notes/?apps_id={$application.id}&amp;page=apps'); return false;" class="action">[[Add Note]]</a>&nbsp;&nbsp;
                        {/if}
                    </td>
                </tr>
                {if $application.note}
                ...
                {else}
                    <tr id="trNote_{$application.id}" style="display: none">
                        <td id="tdNote_{$application.id}" {if $acl->isAllowed('use_screening_questionnaires')}colspan="7"{else}colspan="6"{/if} >
            ...
            {foreachelse}
                <tr>
                    <td colspan="9" class="pointedInListingInfo"><div class="text-center alert alert-info">[[You have no Applications now]]</div></td>
                </tr>
            {/foreach}
        </table>
    </div>
