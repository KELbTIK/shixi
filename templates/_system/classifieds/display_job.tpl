<div id="displayListing">
	...
	<div class="results">
		<div id="topResults">
            ...
            <br/>
			<div class="searchResultsHeaderLineNew sorting-filters">
				<div id="header-searchres-left"></div>
				<ul class="list-inline btn-filter">
					{if $GLOBALS.user_page_uri != "/my-job-details/" && $GLOBALS.user_page_uri != "/job-preview/"}
						{if $acl->isAllowed('save_job')}
							<li class="panelSavedIco">
								<a href="{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}&displayForm=1', 400, '[[Save this Job]]', true, {if $GLOBALS.current_user.logged_in}true{else}false{/if}); return false;"  class="action btn btn-success btn-sm">[[Save this Job]]</a>
							</li>
						{elseif $acl->getPermissionParams('save_job') == "message"}
							<li class="panelSavedIco">
								<a href="{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}"  onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=save_job', 400, '[[Save this Job]]'); return false;" class="action btn btn-success btn-sm">[[Save this Job]]</a>
							</li>
						{/if}
						{if $GLOBALS.current_user.logged_in}
							{if $acl->isAllowed('save_job')}
								<li class="panelViewDitailsIco">
									<a href="{$GLOBALS.site_url}/saved-jobs/" class="action btn btn-primary btn-sm">[[View Saved Jobs]]</a>
								</li>
							{elseif $acl->getPermissionParams('save_job') == "message"}
								<li class="panelViewDitailsIco">
									<a href="{$GLOBALS.site_url}/saved-jobs/" class="action btn btn-primary btn-sm" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=save_job', 400, '[[View Saved Jobs]]'); return false;">[[View Saved Jobs]]</a>
								</li>
							{/if}
						{else}
							<li class="panelViewDitailsIco">
								<a class="btn btn-primary btn-sm" onclick="popUpWindow('{$GLOBALS.site_url}/saved-listings/?listing_type_id=job', 400, 'Saved jobs'); return false;" href="{$GLOBALS.site_url}/saved-listings">[[View Saved Jobs]]</a>
							</li>
						{/if}
					{/if}
					<li class="printListingIco">
						<a class="btn btn-info btn-sm" href="{$GLOBALS.site_url}/tell-friends/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/tell-friends/?listing_id={$listing.id}', 650, '[[Tell a Friend]]'); return false;">[[Tell a Friend]]</a>
					</li>
					{if $acl->isAllowed('flag_job')}
						<li class="printListingIco">
							<a class="btn btn-warning btn-sm" href="{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}', 500, '[[Flag This Job]]'); return false;">[[Flag This Job]]</a>
						</li>
					{elseif $acl->getPermissionParams('flag_job') == "message"}
						<li class="printListingIco">
							<a class="btn btn-warning btn-sm" href="{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=flag_job', 400, '[[Flag This Job]]'); return false;">[[Flag This Job]]</a>
						</li>
					{/if}
					{if $GLOBALS.user_page_uri == "/my-job-details/"}
						<li class="printListingIco">
							<a class="btn btn-dark btn-sm" target="_blank" href="{$GLOBALS.site_url}/print-my-job/?listing_id={$listing.id}">[[Print This Ad]]</a>
						</li>
					{else}
						<li class="printListingIco">
							<a class="btn btn-dark btn-sm" target="_blank" href="{$GLOBALS.site_url}/print-listing/?listing_id={$listing.id}">[[Print This Ad]]</a>
						</li>
					{/if}
					{if $listing.latitude && $listing.longitude && (!$GLOBALS.settings.cookieLaw || $smarty.cookies.cookiePreferences != "System")}
						<li class="viewMapIco">
							<a class="btn btn-default btn-sm" href="{$GLOBALS.site_url}/display-job-map/?listing_id={$listing.id}&amp;searchId={$searchId}&amp;view=map" onclick="popUpWindowIframe('{$GLOBALS.site_url}/display-job-map/?listing_id={$listing.id}&amp;searchId={$searchId}&amp;view=map&amp;lightbox=1', 810, 710, '[[Map]]'); return false;">[[Map View]]</a>
						</li>
					{/if}
				</ul>
			...
			<div class="underQuickLinks">
				...
				<div class="pull-right">
					{if $searchId != "" || $GLOBALS.user_page_uri == "/my-job-details/"}
						<ul class="pagination inline-form">
							<li>
	...
	<div id="refineResults" class="col-sm-4">
		<!-- PROFILE BLOCK -->
        <div class="col-xs-12 gray-bg">
            <div class="userInfo">
                <div id="blockTop"></div>
                <h3 class="compProfileTitle">[[Company Info]]</h3>
                <div class="compProfileInfo">
                    {if $listing.anonymous != 1 || $applications.anonymous === 0 }
                        {if $listing.user.Logo.file_url || $listing.ListingLogo.file_url}
                            <div><img class="img-thumbnail" src="{if $listing.ListingLogo.file_url}{$listing.ListingLogo.file_url}{else}{$listing.user.Logo.file_url}{/if}" alt="" /></div><br/>
                        {/if}
                        <span class="strong"><b>{$listing.user.CompanyName|escape:'html'}</b></span>
	...
	<div id="listingsResults" class="col-sm-8">
		<!-- LISTING INFO BLOCK -->
	...
	<div id="endResults" class="col-xs-12">
        <div class="col-xs-12">
            <ul class="pagination inline-form pull-right">
                {if $searchId != "" || $GLOBALS.user_page_uri == "/my-job-details/"}
