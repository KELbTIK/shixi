...
<form name="SavedListingForm" action="">
	...
	<input type="submit" value="[[Delete]]"	class="button btn btn-danger" onclick="if (confirm('[[Are you sure you want to delete this listing?]]')) submitForm('delete');" /><br/><br/>
            ...
            <tbody>
                ...
                <tr class="{cycle values = 'evenrow,oddrow' advance=true}">
                    ...
                    <td colspan="2">
                        ...
                        {if $listing.saved_listing.note != ''}<a class="btn btn-success radius btn-sm" href="{$GLOBALS.site_url}/edit-notes/?listing_id={$listing.id}" onclick="SaveAd( 'formNote_{$listing.id}', '{$GLOBALS.site_url}/edit-notes/?listing_sid={$listing.id}'); return false;">[[Edit notes]]</a>
                        {else}<a class="btn btn-success radius btn-sm" href="{$GLOBALS.site_url}/add-notes/?listing_id={$listing.id}" onclick="SaveAd( 'formNote_{$listing.id}', '{$GLOBALS.site_url}/add-notes/?listing_sid={$listing.id}'); return false;">[[Add notes]]</a>
                        {/if}
                        </span>
                        <a class="btn btn-danger btn-sm radius" href="?action=delete&amp;listing_id[{$listing.id}]=1">[[Delete]]</a>
                        <a class="btn btn-info radius btn-sm" href="{$GLOBALS.site_url}/{$link}/{$listing.id}/">[[View details]]</a>
                        {if $listing.video.file_url}
                            <a class="btn btn-primary radius btn-sm" onclick="popUpWindow('{$GLOBALS.site_url}/video-player/?listing_id={$listing.id}', 282, 'VideoPlayer'); return false;"  href="{$GLOBALS.site_url}/video-player/?listing_id={$listing.id}">[[Watch a video]]</a>
                        {/if}
