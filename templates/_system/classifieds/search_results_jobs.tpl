	...
	{else}
		...
		<div {if $refineSearch || $view == 'map'}class="results"{else}class="noRefine"{/if}>
			...
			<div id="topResults">
				<div class="headerBgBlock">
					...
					{if $is_company_profile_page}
						...
						{/if}
					{else}
						<h1 class="Results">[[Job Search Results]]</h1>
					{/if}
				...
				{if $view_on_map}
					<div id="googleMap-links">
						<a href="{$GLOBALS.site_url}{if $url == '/search-results-jobs/'}{$url}?searchId={$searchId}&amp;action=search&amp;{else}{$url}?{/if}page=1&amp;view=list" id="listView-icon" {if $view == 'list'}onclick="return false;" {/if} class="listLink-active btn btn-info">[[List View]]</a> &nbsp;
						<a href="{$GLOBALS.site_url}{if $url == '/search-results-jobs/'}{$url}?searchId={$searchId}&amp;action=search&amp;{else}{$url}?{/if}page=1&amp;view=map" id="mapView-icon" {if $view == 'map'}onclick="return false;" {/if} class="listLink-active btn btn-success">[[Map View]]</a>
					</div>
				...
				<div class="topNavBar sorting-filters">
                    <form action="" class="form-inline">
                        <div class="form-group">
                            <label class="numberResults">
                        ...
                        <div class="numberPerPage form-group">
			...
			{if $refineSearch || $view == 'map'}
				<div id="refineResults" class="col-sm-5 col-md-4 sidebar">
					<div id="blockBg">
						<div id="blockTop"></div>
						<div id="blockInner">
			...
			<!-- LISTINGS TABLE -->
			<div id="listingsResults" class="col-sm-7 col-md-8 table-responsive">
				{if $listings}
					{if $view == 'map'}
                            ...
                            <table class="table table-striped table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>
                                            <a href="?searchId={$searchId}&amp;action=search&amp;sorting_field=activation_date&amp;sorting_order={if $listing_search.sorting_order == 'ASC' && $listing_search.sorting_field == 'activation_date'}DESC{else}ASC{/if}&amp;page={$listing_search.current_page}">[[Posted]]
				...
				{else}
					...
					<div class="table-responsive">
                        <table cellspacing="0" id="listingsTableResults" class="table table-striped table-condensed">
                            <thead>
                                <tr>
                                    <th>[[Title]]</th>
                                    <th>[[Company]]</th>
                            ...
                            <tbody>
                                ...
                                <tr id="no_listings_found">
                                    <td colspan="6" style="display:none;"><p class="information alert alert-warning">[[There are no postings meeting the criteria you specified]]</p></td>
                                </tr>
                                {if !$is_company_profile_page}
                                    <!-- preloader row here -->
                                    <tr id="ajax_preloader_listings_results">
                                        <td colspan="6" class="text-center">&nbsp;<img src="{$GLOBALS.site_url}/templates/_system/main/images/ajax_preloader_circular_32.gif" /></td>
                                    </tr>
                                {/if}
			...
			<div id="endResults" class="col-md-offset-4 col-sm-offset-5">
                <div class="col-xs-12">
                    <ul class="pagination">
                        <li class="prevBtn">
                            {if $listing_search.current_page-1 > 0}<a href="?searchId={$searchId}&amp;action=search&amp;page={$listing_search.current_page-1}&amp;view={$view}">[[Previous]]</a>{else}<a>[[Previous]]</a>{/if}
