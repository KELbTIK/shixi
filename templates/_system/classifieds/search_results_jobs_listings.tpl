...
{foreach from=$listings item=listing name=listings}
	{if $listing.api}
		...
		<tr {if $listing.priority == 1}class="priorityListing {$listing.api}Block"{else}class="{cycle values = 'evenrow,oddrow' advance=true} {$listing.api}Block"{/if}>
            ...
            <td>{$listing.CompanyName|escape:'html'}</td>
	...
	{else}
        ...
        <tr>
            ...
            <a href="{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=save_job', 300, '[[Save ad]]'); return false;" class="action">[[Save ad]]</a>&nbsp;&nbsp;
                            ...
                            {/if}
                        {/if}
                        <li class="viewDetails col-sm-6 col-md-4">
                            <div class="row">
                                <a class="btn btn-primary btn-block" href="{$GLOBALS.site_url}/display-job/{$listing.id}/{$listing.Title|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/":"-"}.html?searchId={$searchId}&amp;page={$listing_search.current_page}">[[View job details]]</a>
                            </div>
                        </li>
                        {if $listing.video.file_url}
                            <li class="viewVideo col-sm-6 col-md-4 col-md-offset-4">
                                <div class="row">
                                    <a class="btn btn-warning btn-block" onclick="popUpWindow('{$GLOBALS.site_url}/video-player/?listing_id={$listing.id}&amp;field_id=video', 282, 'VideoPlayer'); return false;"  href="{$GLOBALS.site_url}/video-player/?listing_id={$listing.id}&amp;field_id=video">[[Watch a video]]</a>
                                </div>
                            </li>
                        {/if}
                    </ul>
