...
{if $isRequireConfirmation == 1}
    ...
    <div class="row">
        <div class="col-xs-12">
            <input type="text" {if $editProfile == 1} value="{$value|escape:'html'}" {else} value="{$confirmed|escape:'html'}" {/if} class="inputString form-control" name="{if $complexField}{$complexField}[{$id}][{$complexStep}][confirmed]{else}{$id}[confirmed]{/if}"  onblur="checkField($(this), '{$id}')/><i class="fa fa-envelope form-control-feedback"></i>
        </div>
    </div>
