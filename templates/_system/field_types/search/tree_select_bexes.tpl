...
<script type="text/javascript">
	...
	function makeRequest_{$id}(treeFieldSID, fieldName, parent, elem) {
		$.get("{$GLOBALS.user_site_url}/tree-options/?userTree={$userTree}", { id: treeFieldSID, search: "yes", name: fieldName, parentSID: parent, check: selectedStr_{$id} },
			function(data) {
				elem.html(data);
			}
		);
