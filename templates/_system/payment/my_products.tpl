...
{if $cancelRecurringContractId}
	{foreach from=$contracts_info item=contractInfo}
		{if $contractInfo.id == $cancelRecurringContractId}
			...
			<div class="message alert alert-success">[[Your recurring subscription for product]] [[{$cancelProductName}]] [[was successfully canceled. The product will not renew automatically after expiration.]]</div>
		{/if}
	{/foreach}
{/if}
{if $errors}
	{foreach from=$errors item=error key=message}
		{if $message == 'SUBSCRIPTION_IS_FAIL'}<div class="error alert alert-danger">[[Subscription failed]]</div>{/if}
	{/foreach}
	{elseif $smarty.get.subscriptionComplete}
	<div class="message alert alert-success">[[Your payment was successfully completed. Recurring Subscription activation may take some time. Once subscription is activated it will appear in the table below.]]</div>
{/if}
{if $productsFailed}
	{foreach from=$productsFailed item=product}
	<div class="error alert alert-danger">[[Subscription to product "$product" failed]]</div>
	{/foreach}
{/if}
...
{if $statistics.listingAmount}
	{foreach from=$statistics.listingAmount item=listingAmount}
        <div class="form-group">
            <label class="currentProducts-name col-sm-5">[[{$listingAmount.name}s Left to Post]]:</label>
            <label class="currentProducts-info  col-sm-3">{if $listingAmount.listingsLeft === 0}0{else}[[{$listingAmount.listingsLeft}]]{/if}</label>
...
{if $statistics.avalaibleViews}
	{foreach from=$statistics.avalaibleViews item=avalaibleViews key = listingType}
		{if ($GLOBALS.current_user.group.id != "JobSeeker" && $GLOBALS.current_user.group.id != "Employer") || ($GLOBALS.current_user.group.id == "JobSeeker" && $listingType == 'Job') || ($GLOBALS.current_user.group.id == "Employer" && $listingType == 'Resume')}
            <div class="form-group">
                <label class="currentProducts-name col-sm-5">[[{$avalaibleViews.name}s Left to View]]:</label>
                <label class="currentProducts-info  col-sm-3">{if $avalaibleViews.viewsLeft === 0}0{else}[[{$avalaibleViews.viewsLeft}]]{/if}</label>
                <div class="clearfix"></div>
            </div>
		{/if}
	{/foreach}
...
{if $statistics.avalaibleContactViews}
	{foreach from=$statistics.avalaibleContactViews item=avalaibleContactViews key = listingType}
		{if ($GLOBALS.current_user.group.id != "JobSeeker" && $GLOBALS.current_user.group.id != "Employer") || ($GLOBALS.current_user.group.id == "JobSeeker" && $listingType == 'Job') || ($GLOBALS.current_user.group.id == "Employer" && $listingType == 'Resume')}
            <div class="form-group">
                <label class="currentProducts-name col-sm-5">[[{$avalaibleContactViews.name} Contact details left to view]]:</label>
                <label class="currentProducts-info  col-sm-3">{if $avalaibleContactViews.viewsLeft === 0}0{else}[[{$avalaibleContactViews.viewsLeft}]]{/if}</label>
                <div class="clearfix"></div>
            </div>
		{/if}
	{/foreach}
...
<div class="clearfix"></div>
<div class="alert alert-info">[[To purchase a new product please go to the Products section of the website]]:</div>
<p><a class="btn btn-primary" href="{$GLOBALS.site_url}/products/" class="button">[[Products]]</a></p>