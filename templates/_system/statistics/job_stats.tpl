...
{else}
	<div id="reports">
		...
		<h2>[[Job Stats]]</h2>
		<div id="job-stats">
			<form name="filterForJobsStat" id="filterForJobsStat" method="post" action="{$GLOBALS.site_url}/job-stats/" class="form-inline">
                <label>[[Display]]:</label> &nbsp;
				<select class="form-control" name="active" onchange="submitForm()">
			...
			{else}
				<div class="alert alert-info">[[You have no listings now]] [[Click]] <a href="{$GLOBALS.site_url}/add-listing/?listing_type_id=Job">[[here]]</a> [[to add a new listing.]]</div>
			{/if}
		</div>
	</div>
{/if}
